...
def crash_detection_isr(timer):
    ...
    cool_down_period = 2.0  # Time in seconds

    current_time = time.ticks_ms()/1000  # Time in seconds

    # If not enough time has passed since the last crash, return immediately
    if last_crash_time is not None and (current_time - last_crash_time) < cool_down_period:
        return
    ...
    if accel_data is not None and distance is not None:
        ...
        if jerk > 50 and acc_mag > 6:  # Add gyro condition if needed
            ...
            last_crash_time = current_time  # Update the time of the last crash
            if DEBUG:
